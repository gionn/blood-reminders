...
def index(request):
    not_recent_donation_taken = Q(donation__done_at__lte=datetime.now() - timedelta(days=30))
    donation_never_taken = Q(donation__isnull=True)

    not_recent_reminder_sent = Q(reminder__sent_at__lte=datetime.now() - timedelta(days=7))
